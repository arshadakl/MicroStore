...
import { Store, getTrialDaysRemaining, getSubscriptionDaysRemaining } from "@/types"
import { Clock, AlertTriangle } from "lucide-react"
import { TRIAL_CONFIG, SUBSCRIPTION_CONFIG } from "@/lib/constants"
...
export function TrialBanner({ store }: TrialBannerProps) {
  ...
  if (store.status === "trial") {
    ...
    const isUrgent = daysRemaining <= TRIAL_CONFIG.WARNING_DAYS
        ...
        <div className="bg-destructive/10 border-b border-destructive/20 px-4 py-3">
          <div className="flex flex-wrap items-center gap-2 max-w-4xl mx-auto">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-destructive">Your trial has expired</p>
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3 max-w-4xl mx-auto">
          <Clock className={`w-5 h-5 shrink-0 ${isUrgent ? "text-amber-600" : "text-blue-600"}`} />
          <div className="flex-1 min-w-0">
            <p
            ...
            >
              {isUrgent
                ? "Contact us now to keep your store active."
  ...
  // Active stores with subscription expiry
  if (store.status === "active" && store.subscription_ends_at) {
    const daysRemaining = getSubscriptionDaysRemaining(store.subscription_ends_at)
    const isExpired = daysRemaining <= 0
    const isUrgent = daysRemaining <= SUBSCRIPTION_CONFIG.WARNING_DAYS
        ...
        <div className="bg-destructive/10 border-b border-destructive/20 px-4 py-3">
          <div className="flex flex-wrap items-center gap-2 max-w-4xl mx-auto">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-destructive/80 mt-0.5">
                Contact support to renew and keep your store active.
              </p>
            </div>
            <a
              href="mailto:support@microstore.com?subject=Subscription%20Renewal"
              className="text-sm font-medium text-destructive hover:underline shrink-0"
            >
              Contact Support
            </a>
          ...
          <div className="flex items-center gap-3 max-w-4xl mx-auto">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-amber-700/80 dark:text-amber-300/80 mt-0.5">
                Contact support to renew before your store goes offline.
              </p>
            </div>
            <a
              href="mailto:support@microstore.com?subject=Subscription%20Renewal"
